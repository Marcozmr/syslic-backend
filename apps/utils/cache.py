...
class CacheManager:
    ...
    def clear(self, model, account, suffix='list'):
        if suffix == '':
            keys_pattern = f"syslic.cache.{account}.{model}.*"
        else:
            keys_pattern = f"syslic.cache.{account}.{model}.{suffix}.*"
    ...
    def get_cache_key(self, model, request, pk=''):
        ...
        if (pk != ''):
            key = f"syslic.cache.{account}.{model}.{pk}.{hashPath}.{hashQuery}"
        else:
            key = f"syslic.cache.{account}.{model}.list.{hashPath}.{hashQuery}"
